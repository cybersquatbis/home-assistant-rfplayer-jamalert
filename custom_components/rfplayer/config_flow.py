...
class RfplayerConfigFlow(ConfigFlow):
    ...
    async def async_step_user(self, user_input: dict[str, Any] | None = None) -> ConfigFlowResult:
        ...
        if user_input is not None:
            ...
            if not schema_errors:
                if user_input.get(CONF_DEVICE_SERIAL):
                    device_port = await self.hass.async_add_executor_job(get_serial_by_id, user_input[CONF_DEVICE_SERIAL])
            ...
            if not schema_errors:
                entry_data = {CONF_DEVICE: device_port, CONF_AUTOMATIC_ADD: True, CONF_RECONNECT_INTERVAL: DEFAULT_RECONNECT_INTERVAL, CONF_RECEIVER_PROTOCOLS: DEFAULT_RECEIVER_PROTOCOLS, CONF_INIT_COMMANDS: INIT_COMMANDS_EMPTY, CONF_VERBOSE_MODE: False, CONF_DEVICES: {}, CONF_REDIRECT_ADDRESS: {}}
        ...
        list_of_ports = {}
        for port in ports:
            list_of_ports[port.device] = f"{port}, s/n: {port.serial_number or 'n/a'}" + (f" - {port.manufacturer}" if port.manufacturer else "")
        data_schema = {vol.Optional(CONF_DEVICE_SERIAL): vol.In(list_of_ports), vol.Optional(CONF_IP_ADDRESS): str, vol.Optional(CONF_PORT): int, vol.Optional(CONF_DEVICE_SIMULATOR): bool}
        return self.async_show_form(step_id="user", data_schema=vol.Schema(data_schema), errors=schema_errors)
...
class RfPlayerOptionsFlowHandler(OptionsFlow):
    ...
    async def async_step_init(self, user_input: dict[str, Any] | None = None) -> ConfigFlowResult:
        ...
        return self.async_show_menu(step_id="init", menu_options={"configure_gateway": "RfPlayer gateway options", "configure_rf_device": "RF device options", "add_rf_device": "Add RF device"})

    async def async_step_configure_gateway(self, user_input: dict[str, Any] | None = None) -> ConfigFlowResult:
        errors: dict[str, Any] = {}
        if user_input is not None:
            global_options = {CONF_AUTOMATIC_ADD: user_input[CONF_AUTOMATIC_ADD], CONF_RECONNECT_INTERVAL: user_input[CONF_RECONNECT_INTERVAL], CONF_RECEIVER_PROTOCOLS: user_input[CONF_RECEIVER_PROTOCOLS], CONF_INIT_COMMANDS: user_input.get(CONF_INIT_COMMANDS, INIT_COMMANDS_EMPTY), CONF_VERBOSE_MODE: user_input[CONF_VERBOSE_MODE]}
        ...
        options = {vol.Required(CONF_AUTOMATIC_ADD, default=data[CONF_AUTOMATIC_ADD]): bool, vol.Required(CONF_RECONNECT_INTERVAL, default=data[CONF_RECONNECT_INTERVAL]): int, vol.Required(CONF_RECEIVER_PROTOCOLS, default=data[CONF_RECEIVER_PROTOCOLS]): cv.multi_select(RECEIVER_MODES), vol.Optional(CONF_INIT_COMMANDS, description={"suggested_value": data[CONF_INIT_COMMANDS]}): str, vol.Required(CONF_VERBOSE_MODE, default=data[CONF_VERBOSE_MODE]): bool}
    ...
    async def async_step_configure_rf_device(self, user_input: dict[str, Any] | None = None) -> ConfigFlowResult:
        ...
        if user_input is not None:
            ...
            if not errors:
                ...
                return self.async_create_entry(title="", data={})
        option_schema = {vol.Required(CONF_DEVICE): vol.In(self._list_rf_devices()), vol.Optional(CONF_REDIRECT_ADDRESS): str}
    ...
    async def async_step_add_rf_device(self, user_input: dict[str, Any] | None = None) -> ConfigFlowResult:
        ...
        if user_input is not None:
            ...
            if not errors:
                ...
                return self.async_create_entry(title="", data={})
        data = self.config_entry.data
        option_schema = {vol.Required(CONF_PROTOCOL, default=data.get(CONF_PROTOCOL)): vol.In(DEVICE_PROTOCOLS), vol.Required(CONF_ADDRESS, default=data.get(CONF_ADDRESS)): str, vol.Required(CONF_PROFILE_NAME, default=data.get(CONF_PROFILE_NAME)): vol.In(profile_registry.get_profile_names())}
