...
class RfDeviceId:
    ...
    @property
    def integer_address(self) -> int: return self._integer_address
    @property
    def id_string(self) -> str: return f"{self.protocol}-{self.integer_address}"
    @property
    def group_code(self) -> str | None: return str(self.integer_address & 0xFFFFFF00)
    @property
    def unit_code(self) -> str | None: return str(self.integer_address & 0x000000FF)
    @staticmethod
    def is_valid_address(address: str) -> bool:
        try: RfDeviceId._address_to_integer(address)
    ...
    def _address_to_integer(address: str) -> int:
        ...
        if m: return int(upper_address)
        m = RfDeviceId.X10_PATTERN.fullmatch(upper_address)
        if m: return ((ord(m.group(1)) - ord("A")) * 16 + int(m.group(2))) - 1
...
class RfDeviceEventAdapter:
    ...
    def _convert_raw_model(self, raw_model: str) -> str:
        return "switch" if raw_model.lower() in ["on", "off"] else raw_model
    def _get_model(self, infos: dict[str, Any]) -> str:
        for key in ["id_PHYMeaning", "subTypeMeaning"]:
            if key in infos: return self._convert_raw_model(infos[key])
        return UNKNOWN_INFO
    def _get_address(self, infos: dict[str, Any], protocol: str | None = None) -> str:
        """Extract address; JAMMING is a receiver event and uses synthetic address 0."""
        for key in ["id", "id_channel", "adr_channel"]:
            if key in infos: return str(infos[key])
        if protocol == "JAMMING": return "0"
        return UNKNOWN_INFO
    def _parse_json_device(self, json_packet: dict[str, Any]) -> RfDeviceId:
        ...
        return RfDeviceId(protocol=protocol, address=self._get_address(infos, protocol), model=self._get_model(infos))
